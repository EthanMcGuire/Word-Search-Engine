...
void NineSlice::render(SDL_Renderer *renderer, int x, int y, SDL_Rect *clip, double angle, SDL_Point *center, SDL_RendererFlip flip, int alpha, gmtl::Vec3i colorMod) const
{
...
/// @param center Center point to rotate the texture at. Defaults at w/2, h/2 of the dest rect (image center).
/// @param flip Whether to flip each slice horizontally or vertically.
void NineSlice::drawSlice(SDL_Renderer *renderer, unsigned int spriteIndex, int x, int y, unsigned int w, unsigned int h, SDL_Rect *clip, double angle, SDL_Point *center, SDL_RendererFlip flip) const
{
    if (w == 0 || h == 0)
    {
        return;

...
void SpriteAtlas::render(SDL_Renderer *renderer, unsigned int spriteIndex, int x, int y, double xScale, double yScale, SDL_Rect *clip, double angle, SDL_Point *center, SDL_RendererFlip flip) const
{
    if (!textureLoaded())
    {
...
/// @param center Center point to rotate the texture at. Defaults at w/2, h/2 of the dest rect (image center).
/// @param flip Whether the flip the texture horizontally or vertically.
void SpriteAtlas::renderStretched(SDL_Renderer *renderer, unsigned int spriteIndex, int x, int y, int drawWidth, int drawHeight, SDL_Rect *clip, double angle, SDL_Point *center, SDL_RendererFlip flip) const
{
    if (!textureLoaded())
    {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "SpriteAtlas: Attempted to render sprite with no load sprite atlas! Call loadTexture() first.");

...
bool DrawingSurface::createSurface(SDL_Renderer *renderer, unsigned int w, unsigned int h)
{
    ...
    texture = SDL_CreateTexture(renderer, SDL_PixelFormat::SDL_PIXELFORMAT_RGBA8888, SDL_TextureAccess::SDL_TEXTUREACCESS_TARGET, w, h);
...
void DrawingSurface::drawSurface(SDL_Renderer *renderer, int x, int y, double xScale, double yScale, SDL_FRect *clip, double angle, SDL_FPoint *center, SDL_FlipMode flip) const
{
    if (!surfaceExists())
    {
...
/// @param center Center point to rotate the texture at. Defaults at w/2, h/2 of the dest rect (image center).
/// @param flip Whether to flip the texture horizontally or vertically.
void DrawingSurface::drawSurfaceStretched(SDL_Renderer *renderer, int x, int y, int drawWidth, int drawHeight, SDL_FRect *clip, double angle, SDL_FPoint *center, SDL_FlipMode flip) const
{
    if (!surfaceExists())
    {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "DrawingSurface: Failed to draw the drawing surface. Create the surface first with createSurface()");
...
/// @brief Sets the scale mode for the surface.
/// @param scaleMode The scale mode.
void DrawingSurface::setScaleMode(SDL_ScaleMode scaleMode)
{
    if (!surfaceExists())
    {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "DrawingSurface: Failed to set scale mode for the drawing surface. Create the surface first with createSurface()");
